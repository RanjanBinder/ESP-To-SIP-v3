...
const UpdateEspPage = ({ station, setStation, selectedEsp, setSelectedEsp, onGoLibrary, onOpenEditor, onExit }) => {
  ...
  const stations = STATIONS.filter((s) => (!division || s.division === division) && (!section || s.section === section) &&
    (!stationSearch.trim() || s.name.toLowerCase().includes(stationSearch.trim().toLowerCase()) || s.code.toLowerCase().includes(stationSearch.trim().toLowerCase())));
  ...
  return h("div", { className: "esp-page" },
    ...
    h("div", { className: "esp-body" }, h("div", { className: "esp-update-page" },
      h("div", { className: "esp-sec" }, h(SecTitle, null, "Select Station"), h(Card, null,
        h("div", { className: "esp-update-grid" },
          h(Field, { label: "Zone" }, h(Select, { value: "SCR", disabled: true }, h("option", null, "SCR — South Central Railway"))),
          h(Field, { label: "Division" }, h(Select, { value: division, onChange: (e) => { setDivision(e.target.value); setSection(""); chooseStation(""); } }, h("option", { value: "" }, "All divisions"), divisions.map((v) => h("option", { key: v, value: v }, v)))),
          h(Field, { label: "Section" }, h(Select, { value: section, onChange: (e) => { setSection(e.target.value); chooseStation(""); } }, h("option", { value: "" }, "All sections"), sections.map((v) => h("option", { key: v, value: v }, v)))),
          ...
          h(Field, { label: "Station", required: true }, h(Select, { value: station ? station.code : "", onChange: (e) => chooseStation(e.target.value) }, h("option", { value: "" }, "Select station"), stations.map((s) => h("option", { key: s.code, value: s.code }, `${s.name} (${s.code})`))))
        )
      )),
      station && h("div", { className: "esp-sec" }, h(SecTitle, { n: `${espRows.length} drawing${espRows.length === 1 ? "" : "s"}` }, "Select ESP"),
        espRows.length ? h(Card, { bodyStyle: { padding: 0 } }, h("div", { className: "esp-tablewrap" }, h("table", { className: "esp-table" },
          ...
          h("tbody", null, espRows.map((row) => h("tr", { key: row.drawing, onClick: () => setSelectedEsp(row.drawing), style: { cursor: "pointer", background: selectedEsp === row.drawing ? "var(--accent-soft)" : undefined } },
            h("td", null, h(Radio, { checked: selectedEsp === row.drawing })), h("td", null, row.file), h("td", null, h("span", { className: "esp-td-station" }, row.drawing)), h("td", null, h("span", { className: "esp-ver" }, row.version)), h("td", null, h(Chip, { tone: row.status === "Approved" ? "success" : "neutral", dot: true }, row.status)), h("td", null, row.updated)
          )))
      ...
      station && espRows.length > 0 && h("div", { className: "esp-update-actions" }, h(Btn, { variant: "primary", trailingIcon: "arrow_right", disabled: !selectedEsp, onClick: onOpenEditor }, "Open in Editor"))
    ))
  );
};
...
const EspModulePage = ({ onNavigate }) => {
  ...
  const startFlow = (f) => {
    setFlow(f); setStep(0); setMaxReached(0); setView("workflow");
    setStation(null);
  };
  ...
  if (view === "workflow" && flow === "update" && step < 5) {
    return <UpdateEspPage station={station} setStation={setStation} selectedEsp={prevEsp} setSelectedEsp={setPrevEsp}
      onGoLibrary={() => onNavigate && onNavigate("library")} onOpenEditor={() => goStep(5)} onExit={() => setView("landing")} />;
  }
  ...
  return (
    <div className="esp-page">
      ...
      <div className="esp-stepbar">
        <div className="esp-steps">
          {WORKFLOW_STEPS.map((s, i) => {
            const state = i < step ? "done" : i === step ? "active" : "pending";
            const locked = i > maxReached;
            return (
              <div key={s.id} className="esp-step" data-state={state} data-locked={locked}
                   onClick={() => !locked && goStep(i)}>
                <div className="esp-step-line" />
                <div className="esp-step-dot">{state === "done" ? <Icon name="check" size={12} /> : i + 1}</div>
                <div className="esp-step-txt">
                  <div className="esp-step-label">{s.label}</div>
                  <div className="esp-step-sub">
                    {flow === "update" && i === 1 ? "Previous ESP" : flow === "update" && i === 2 ? "Proposed changes" : s.sub}
                  </div>
                </div>
              </div>
            );
