// Generated from pim-track-correction.jsx. Edit the .jsx source and regenerate if needed.
(() => {
  (() => {
    ...
    const chainageAt = (x, station = D.STATION) => (stationBaseChainage(station) + (x - 60) / 1e3).toFixed(3);
    ...
    const lineLength = (coordinates) => (coordinates || []).reduce((total, point, index) => index ? total + distance(coordinates[index - 1], point) : total, 0);
    ...
    const combinePaths = (paths) => {
      ...
      while (pool.length) {
        ...
        let best = null;
        pool.forEach((path2, index) => {
          const candidates = [
            { index, where: "tail", reverse: false, gap: distance(tail, path2[0]) },
            { index, where: "tail", reverse: true, gap: distance(tail, path2[path2.length - 1]) },
            { index, where: "head", reverse: false, gap: distance(head, path2[path2.length - 1]) },
            { index, where: "head", reverse: true, gap: distance(head, path2[0]) }
          ];
    ...
    const buildInitialTopology = (tracks) => {
      ...
      const seenTrackPairs = /* @__PURE__ */ new Set();
    ...
    const isReviewDocument = (value) => value?.schemaVersion === SCHEMA_VERSION && Array.isArray(value.tracks) && value.tracks.every((track) => track?.id && track.geometry?.type === "LineString" && !validateCoordinates(track.geometry.coordinates)) && Array.isArray(value.unmapped) && Array.isArray(value.topology) && Array.isArray(value.archive) && Array.isArray(value.audit);
    ...
    const hydrateReviewDocument = (value, station, sourceDocument) => {
      ...
      hydrated.tracks = hydrated.tracks.map((track) => {
        ...
        const originalGeometry = track.originalGeometry?.type === "LineString" && !validateCoordinates(track.originalGeometry.coordinates) ? { type: "LineString", coordinates: clone(track.originalGeometry.coordinates) } : clone(geometry);
        return {
          ...track,
    ...
    const lineIntersection = (a, b, c, d) => {
      ...
      const inside = (point2, one, two) => point2[0] >= Math.min(one[0], two[0]) - 0.1 && point2[0] <= Math.max(one[0], two[0]) + 0.1 && point2[1] >= Math.min(one[1], two[1]) - 0.1 && point2[1] <= Math.max(one[1], two[1]) + 0.1;
      const point = [x, y];
    ...
    const buildSnapTargets = (tracks) => {
      ...
      const segments = tracks.filter((track) => track.status !== "REJECTED").flatMap((track) => track.geometry.coordinates.slice(0, -1).map((point, index) => ({ trackId: track.id, a: point, b: track.geometry.coordinates[index + 1] })));
    ...
    const detachEndpointTopology = (document2, assetId, endpointIndex) => {
      const track = document2.tracks.find((item) => item.id === assetId);
      const endpointRole = track ? endpointRoleForIndex(track, endpointIndex) : null;
      const touches = (relation) => relationTouchesEndpoint(relation, "source", assetId, endpointIndex, endpointRole) || relationTouchesEndpoint(relation, "destination", assetId, endpointIndex, endpointRole);
      const detachedIds = unique(document2.topology.filter(touches).map((relation) => relation.sourceAssetId === assetId ? relation.destinationAssetId : relation.sourceAssetId));
      document2.topology = document2.topology.filter((relation) => !touches(relation));
      const source = document2.tracks.find((track2) => track2.id === assetId);
      if (source) source.connectedAssetIds = source.connectedAssetIds.filter((id) => !detachedIds.includes(id));
      document2.tracks.forEach((track2) => {
        if (detachedIds.includes(track2.id)) track2.connectedAssetIds = track2.connectedAssetIds.filter((id) => id !== assetId);
      });
      return detachedIds;
    };
    const refreshConnectivity = (document2, assetIds = null) => {
      const touched = assetIds ? new Set(assetIds) : null;
      document2.tracks.forEach((track) => {
        ...
        const hasEndpointRelation = (endpointIndex) => {
          ...
          return document2.topology.some((relation) => relationTouchesEndpoint(relation, "source", track.id, endpointIndex, role) || relationTouchesEndpoint(relation, "destination", track.id, endpointIndex, role));
        };
    ...
    const syncConnectedAssetIds = (document2) => {
      document2.tracks.forEach((track) => {
        track.connectedAssetIds = unique(document2.topology.flatMap((relation) => relation.sourceAssetId === track.id ? [relation.destinationAssetId] : relation.destinationAssetId === track.id ? [relation.sourceAssetId] : []));
      });
    };
    const ptcCSS = `
...
`;
    const ToolButton = ({ icon, label, active, disabled, onClick }) => /* @__PURE__ */ React.createElement(
      "button",
      {
        type: "button",
        className: "ptc-tool",
    ...
    const AddTrackMenu = ({ toolbar, onDraw, onMap }) => /* @__PURE__ */ React.createElement("div", { className: "ptc-add-menu", "data-toolbar": toolbar ? "true" : "false", role: "menu", "aria-label": "Add Track" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-add-title" }, "Add Track"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-add-option", role: "menuitem", onClick: onDraw }, /* @__PURE__ */ React.createElement(Icon, { name: "edit", size: 15 }), /* @__PURE__ */ React.createElement("span", null, "Draw Missing Track", /* @__PURE__ */ React.createElement("small", null, "Click points on the ESP and double-click to finish."))), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-add-option", role: "menuitem", onClick: onMap }, /* @__PURE__ */ React.createElement(Icon, { name: "target", size: 15 }), /* @__PURE__ */ React.createElement("span", null, "Map Existing Geometry", /* @__PURE__ */ React.createElement("small", null, "Select an unidentified source line or polyline."))));
    const TrackToolbar = ({
      mode,
      selectedCount,
      snapEnabled,
      ...
      return /* @__PURE__ */ React.createElement("div", { className: "ptc-toolbar", role: "toolbar", "aria-label": "Track canvas tools" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-tool-group" }, /* @__PURE__ */ React.createElement(ToolButton, { icon: "hand", label: "Pan", active: mode === "PAN", onClick: () => onMode("PAN") }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "zoom_in", label: "Zoom In", onClick: onZoomIn }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "zoom_out", label: "Zoom Out", onClick: onZoomOut }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "fit_screen", label: "Fit View", onClick: onFit })), /* @__PURE__ */ React.createElement("span", { className: "ptc-tool-divider" }), /* @__PURE__ */ React.createElement("div", { className: "ptc-tool-group" }, /* @__PURE__ */ React.createElement(ToolButton, { icon: "cursor", label: "Select", active: mode === "SELECT", onClick: () => onMode("SELECT") }), /* @__PURE__ */ React.createElement("div", { className: "ptc-add-wrap" }, /* @__PURE__ */ React.createElement(ToolButton, { icon: "plus", label: "Add Track", active: addOpen, onClick: onAddToggle }))), /* @__PURE__ */ React.createElement("span", { className: "ptc-tool-divider" }), /* @__PURE__ */ React.createElement("div", { className: "ptc-tool-group" }, /* @__PURE__ */ React.createElement(ToolButton, { icon: "edit", label: "Edit Geometry \u2014 drag control points; double-click a segment to add a point", active: mode === "EDIT_GEOMETRY", disabled: !one, onClick: onEdit }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "arrow_right", label: "Extend Track \u2014 choose an endpoint and destination", active: mode === "EXTEND", disabled: !one, onClick: onExtend }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "scissors", label: "Trim Track \u2014 click the trim location", active: mode === "TRIM", disabled: !one, onClick: onTrim }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "link", label: "Join Tracks \u2014 merge two disconnected track segments", disabled: !two, onClick: onJoin }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "unlink", label: "Split Track \u2014 click the split location", active: mode === "SPLIT", disabled: !one, onClick: onSplit }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "branch", label: "Connect \u2014 store a track or turnout topology connection", active: mode === "CONNECT", disabled: selectedCount < 1 || selectedCount > 2, onClick: onConnect }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "trash", label: selectedCount ? "Reject or delete selected Track" : "Select a Track to reject", disabled: !selectedCount || selectedCount > 2, onClick: onReject })), /* @__PURE__ */ React.createElement("span", { className: "ptc-tool-divider" }), /* @__PURE__ */ React.createElement("div", { className: "ptc-tool-group" }, /* @__PURE__ */ React.createElement(ToolButton, { icon: "target", label: `Snap ${snapEnabled ? "ON" : "OFF"} \u2014 endpoints, control points, turnouts and intersections`, active: snapEnabled, onClick: onSnap }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "undo", label: "Undo (Ctrl/Cmd + Z)", disabled: !canUndo, onClick: onUndo }), /* @__PURE__ */ React.createElement(ToolButton, { icon: "redo", label: "Redo (Ctrl/Cmd + Shift + Z)", disabled: !canRedo, onClick: onRedo })), addOpen && /* @__PURE__ */ React.createElement(AddTrackMenu, { toolbar: true, onDraw, onMap }));
    };
    const MappingForm = ({ geometry, onCancel, onMap, onReject }) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { className: "ptc-map-form" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-map-head" }, /* @__PURE__ */ React.createElement(Icon, { name: "target", size: 16 }), /* @__PURE__ */ React.createElement("div", { className: "ptc-map-head-copy" }, /* @__PURE__ */ React.createElement("strong", null, "Map Existing Geometry"), /* @__PURE__ */ React.createElement("span", null, geometry.label, " \xB7 ", geometry.sourceElementIds.join(", ")))), /* @__PURE__ */ React.createElement("div", { className: "ptc-form-grid" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-field", "data-full": "true" }, /* @__PURE__ */ React.createElement("label", null, "Track Name"), /* @__PURE__ */ React.createElement("input", { value: value.name, autoFocus: true, onChange: (event) => setValue((current) => ({ ...current, name: event.target.value })), placeholder: "e.g. DN MAIN LINE" })), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "Track Type"), /* @__PURE__ */ React.createElement("select", { value: value.trackType, onChange: (event) => setValue((current) => ({ ...current, trackType: event.target.value })) }, TRACK_TYPES.map(([code, label]) => /* @__PURE__ */ React.createElement("option", { key: code, value: code }, label)))), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "Direction"), /* @__PURE__ */ React.createElement("select", { value: value.direction, onChange: (event) => setValue((current) => ({ ...current, direction: event.target.value })) }, DIRECTIONS.map(([code, label]) => /* @__PURE__ */ React.createElement("option", { key: code, value: code }, label))))), /* @__PURE__ */ React.createElement("div", { className: "ptc-form-actions" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-danger": "true", onClick: () => onReject(geometry.suggestedReason || "Not a Railway Track") }, "Not a Track"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", onClick: onCancel }, "Cancel"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-primary": "true", disabled: !value.name.trim(), onClick: () => onMap(value) }, "Map Track")));
    };
    const TrackCard = ({
      track,
      index,
      ...
      return /* @__PURE__ */ React.createElement("article", { className: "ptc-card", "data-active": active ? "true" : "false", "data-rejected": track.status === "REJECTED" ? "true" : "false", ref: cardRef }, /* @__PURE__ */ React.createElement("div", { className: "ptc-card-head", onClick: (event) => onSelect(track.id, event.shiftKey), role: "button", tabIndex: 0, onKeyDown: (event) => {
        if (event.key === "Enter" || event.key === " ") onSelect(track.id, event.shiftKey);
      } }, /* @__PURE__ */ React.createElement("span", { className: "ptc-card-index" }, index + 1), /* @__PURE__ */ React.createElement("div", { className: "ptc-card-heading" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-card-name" }, track.name || "Unnamed Track"), /* @__PURE__ */ React.createElement("div", { className: "ptc-card-meta" }, /* @__PURE__ */ React.createElement("span", { className: "mono" }, track.id), /* @__PURE__ */ React.createElement("span", { className: "ptc-badge", "data-status": track.status }, track.status === "VERIFIED" && /* @__PURE__ */ React.createElement(Icon, { name: "check", size: 10 }), statusLabel(track.status)))), /* @__PURE__ */ React.createElement("div", { className: "ptc-card-tools" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-card-icon", title: "Locate Track", "aria-label": `Locate ${track.name || track.id}`, onClick: (event) => {
        event.stopPropagation();
        onLocate(track.id);
      } }, /* @__PURE__ */ React.createElement(Icon, { name: "target", size: 14 })), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-card-icon", title: track.source === "AI_EXTRACTION" ? "Reject Track" : "Delete Track", "aria-label": track.source === "AI_EXTRACTION" ? `Reject ${track.name || track.id}` : `Delete ${track.name || track.id}`, disabled: track.status === "REJECTED", onClick: (event) => {
        event.stopPropagation();
        onReject(track.id);
      } }, /* @__PURE__ */ React.createElement(Icon, { name: "trash", size: 14 })), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-card-icon", title: expanded ? "Collapse" : "Expand", "aria-label": expanded ? "Collapse Track card" : "Expand Track card", onClick: (event) => {
        event.stopPropagation();
        onToggle(track.id);
      } }, /* @__PURE__ */ React.createElement(Icon, { name: expanded ? "chevron_up" : "chevron_down", size: 14 })))), expanded && /* @__PURE__ */ React.createElement("div", { className: "ptc-card-body" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-form-grid" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-field", "data-full": "true" }, /* @__PURE__ */ React.createElement("label", null, "Track Name"), /* @__PURE__ */ React.createElement("input", { value: draft.name, onChange: set("name"), onBlur: () => commit("name"), list: "ptc-track-names", disabled: track.status === "REJECTED" })), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "Track Type"), /* @__PURE__ */ React.createElement("select", { value: draft.trackType, onChange: (event) => {
        set("trackType")(event);
        onCommitField(track.id, "trackType", event.target.value);
      }, disabled: track.status === "REJECTED" }, TRACK_TYPES.map(([code, label]) => /* @__PURE__ */ React.createElement("option", { key: code, value: code }, label)))), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "Direction"), /* @__PURE__ */ React.createElement("select", { value: draft.direction, onChange: (event) => {
        set("direction")(event);
        onCommitField(track.id, "direction", event.target.value);
      }, disabled: track.status === "REJECTED" }, DIRECTIONS.map(([code, label]) => /* @__PURE__ */ React.createElement("option", { key: code, value: code }, label)))), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "Start Chainage"), /* @__PURE__ */ React.createElement("input", { value: draft.startChainage, onChange: set("startChainage"), onBlur: () => commit("startChainage"), disabled: track.status === "REJECTED" })), /* @__PURE__ */ React.createElement("div", { className: "ptc-field" }, /* @__PURE__ */ React.createElement("label", null, "End Chainage"), /* @__PURE__ */ React.createElement("input", { value: draft.endChainage, onChange: set("endChainage"), onBlur: () => commit("endChainage"), disabled: track.status === "REJECTED" }))), /* @__PURE__ */ React.createElement("div", { className: "ptc-readonly" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-readonly-item" }, /* @__PURE__ */ React.createElement("span", null, "Source"), /* @__PURE__ */ React.createElement("strong", null, formatSource(track.source))), /* @__PURE__ */ React.createElement("div", { className: "ptc-readonly-item" }, /* @__PURE__ */ React.createElement("span", null, "Confidence"), /* @__PURE__ */ React.createElement("strong", null, track.confidence == null ? "\u2014" : `${track.confidence}%`)), /* @__PURE__ */ React.createElement("div", { className: "ptc-readonly-item" }, /* @__PURE__ */ React.createElement("span", null, "Geometry"), /* @__PURE__ */ React.createElement("strong", null, track.geometry.coordinates.length, " points \xB7 ", lineLength(track.geometry.coordinates).toFixed(1), " m")), /* @__PURE__ */ React.createElement("div", { className: "ptc-readonly-item" }, /* @__PURE__ */ React.createElement("span", null, "Connections"), /* @__PURE__ */ React.createElement("strong", null, track.connectedAssetIds.length ? track.connectedAssetIds.join(", ") : "None"))), !!track.issues.length && /* @__PURE__ */ React.createElement("div", { className: "ptc-issues" }, track.issues.map((issue) => /* @__PURE__ */ React.createElement("span", { className: "ptc-issue-chip", key: issue }, issueLabel(issue)))), track.status === "REJECTED" && /* @__PURE__ */ React.createElement("div", { className: "ptc-rejection-note" }, /* @__PURE__ */ React.createElement("strong", null, "Rejected:"), " ", track.rejectionReason || "Incorrect extraction", /* @__PURE__ */ React.createElement("br", null), track.rejectedBy, " \xB7 ", track.rejectedAt), /* @__PURE__ */ React.createElement("div", { className: "ptc-card-actions" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-danger": "true", disabled: track.status === "REJECTED", onClick: () => onReject(track.id) }, track.source === "AI_EXTRACTION" ? "Reject Track" : "Delete Track"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-primary": "true", disabled: track.status === "REJECTED" || track.status === "VERIFIED", onClick: () => onConfirm(track.id) }, /* @__PURE__ */ React.createElement(Icon, { name: "check", size: 12 }), "Confirm Track"))));
    };
    const RejectDialog = ({ tracks, onClose, onConfirm }) => {
      ...
      return /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-backdrop", role: "presentation", onMouseDown: (event) => {
        if (event.target === event.currentTarget) onClose();
      } }, /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog", role: "dialog", "aria-modal": "true", "aria-labelledby": "ptc-reject-title" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-head" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-icon" }, /* @__PURE__ */ React.createElement(Icon, { name: "trash", size: 16 })), /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-copy" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-title", id: "ptc-reject-title" }, manualOnly ? "Delete manually added Track?" : `Reject extracted Track${tracks.length > 1 ? "s" : ""}?`), /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-sub" }, tracks.map((track) => track.id).join(", "), ". Extracted geometry and audit history are preserved when rejected."))), !manualOnly && /* @__PURE__ */ React.createElement("div", { className: "ptc-reasons" }, REJECTION_REASONS.map((item) => /* @__PURE__ */ React.createElement("label", { className: "ptc-reason", key: item }, /* @__PURE__ */ React.createElement("input", { type: "radio", name: "ptc-reason", value: item, checked: reason === item, onChange: () => setReason(item) }), item))), /* @__PURE__ */ React.createElement("div", { className: "ptc-dialog-actions" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", onClick: onClose }, "Cancel"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-danger": "true", onClick: () => onConfirm(manualOnly ? "Deleted manual draft" : reason) }, manualOnly ? "Delete" : "Reject"))));
    };
    const PIMTrackCorrectionWorkspace = ({
      ...
      useEffect(() => {
        ...
        saveTimer.current = window.setTimeout(() => {
          ...
          if (didSave) setLastSavedAt((/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
        }, 360);
      ...
      useEffect(() => {
        ...
        const issueCount2 = issueTracks.length + documentState.unmapped.length;
        const status = !unresolved.length && !documentState.unmapped.length ? "completed" : issueCount2 ? "has_issues" : "incomplete";
        onStatusChange?.({
          status,
          count: active.length,
          total: active.length,
          issueCount: issueCount2,
      ...
      const selectTrack = useCallback((trackId, additive = false) => {
        ...
        setSelectedIds((current) => additive ? current.includes(trackId) ? current.filter((id) => id !== trackId) : [...current, trackId] : [trackId]);
      ...
      const createDrawnTrack = useCallback((points) => {
        ...
        commit({ action: "TRACK_CREATED", assetId: id, previousValue: null, newValue: track, source: "MANUAL" }, (next) => {
      ...
      const rejectUnmappedGeometry = useCallback((reason) => {
        ...
        commit({
          ...
          next.unmapped = next.unmapped.filter((item) => item.id !== geometry.id);
          next.archive.push({ ...geometry, status: "NOT_A_TRACK", rejectionReason: reason, archivedAt: (/* @__PURE__ */ new Date()).toISOString() });
        });
        setMappingTargetId(null);
        setMode("SELECT");
      ...
      const requestReject = useCallback((ids = selectedIds) => {
        ...
        if (!candidates.length) {
          showToast("Select a valid Track first.", "danger");
          return;
        }
        clearTransient();
        setMode("SELECT");
        ...
        commit({
          ...
          candidates.forEach((candidate) => {
            ...
            if (target.source !== "AI_EXTRACTION") {
              next.archive.push({ ...target, archivedAction: "DELETED", archivedAt: (/* @__PURE__ */ new Date()).toISOString() });
              ...
              target.rejectedAt = (/* @__PURE__ */ new Date()).toLocaleString();
              target.version = (target.version || 1) + 1;
            }
          });
      ...
      const confirmTrack = useCallback((trackId) => {
        ...
        setMode("SELECT");
        if (!track.name.trim()) {
          showToast("Track Name is required before confirmation.", "danger");
          return;
        }
        const geometryError = validateCoordinates(track.geometry.coordinates);
        if (geometryError) {
          showToast(geometryError, "danger");
          return;
        }
        ...
        const validTopologyIds = /* @__PURE__ */ new Set([
        ...
        if (invalidReference) {
          showToast(`Connection target ${invalidReference} does not exist.`, "danger");
          return;
        }
      ...
      const startEditGeometry = useCallback(() => {
        if (selectedIds.length !== 1) {
          showToast("Select one Track to edit its geometry.", "danger");
          return;
        }
      ...
      const startExtend = useCallback(() => {
        if (selectedIds.length !== 1) {
          showToast("Select one Track to extend.", "danger");
          return;
        }
        changeMode("EXTEND");
      ...
      const startTrim = useCallback(() => {
        if (selectedIds.length !== 1) {
          showToast("Select one Track to trim.", "danger");
          return;
        }
        changeMode("TRIM");
      ...
      const startSplit = useCallback(() => {
        if (selectedIds.length !== 1) {
          showToast("Select one Track to split.", "danger");
          return;
        }
        changeMode("SPLIT");
      ...
      const startJoin = useCallback(() => {
        if (selectedIds.length !== 2) {
          showToast("Select two Track segments to join.", "danger");
          return;
        }
        const [first, second] = selectedTracks;
        if (!first || !second || first.status === "REJECTED" || second.status === "REJECTED") {
          showToast("Select two valid Tracks to join.", "danger");
          return;
        }
      ...
      const startConnect = useCallback(() => {
        if (selectedIds.length < 1 || selectedIds.length > 2) {
          showToast("Select one or two Tracks to connect.", "danger");
          return;
        }
        changeMode("CONNECT");
      ...
      const removeControlPoint = useCallback((trackId, pointIndex) => {
        ...
        if (!track || track.geometry.coordinates.length <= 2) {
          showToast("A Track must keep at least two points.", "danger");
          return;
        }
      ...
      const chooseTrim = useCallback((track, rawPoint) => {
        ...
        if (!split) {
          showToast("Choose a point away from the Track endpoints.", "danger");
          return;
        }
      ...
      const chooseSplit = useCallback((track, rawPoint) => {
        const projection = projectPointToPolyline(rawPoint, track.geometry.coordinates);
        const split = splitCoordinatesAt(track.geometry.coordinates, projection);
        if (!split || lineLength(split.left) < 1 || lineLength(split.right) < 1) {
          showToast("Choose a valid split point away from the endpoints.", "danger");
          return;
        }
      ...
      const confirmPendingOperation = useCallback(() => {
        ...
        if (operationConfirm.type === "JOIN") {
          ...
          commit({
            ...
            next.archive.push({ ...retired, archivedAction: "MERGED", mergedInto: first.id, archivedAt: (/* @__PURE__ */ new Date()).toISOString() });
            ...
            next.topology = next.topology.map((relation) => {
              ...
              ["source", "destination"].forEach((side) => {
                ...
                if (role === "END") {
                  ...
                  const belongsRight = leftProjection && rightProjection ? rightProjection.distance + 1e-6 < leftProjection.distance : mapped[segmentKey] > operationConfirm.splitSegmentIndex;
                  if (belongsRight) {
                    mapped[assetKey] = newId;
        ...
        if (mode === "CONNECT") {
          if (!connectDraft) {
            if (!selectedIds.includes(trackId)) {
              showToast("Choose an endpoint on a selected source Track.", "danger");
              return;
            }
          ...
          if (connectDraft.trackId === trackId) {
            showToast("Choose a different Track or a turnout.", "danger");
            return;
          }
          const targetPoint = track.geometry.coordinates[pointIndex];
          const sourceTrack = tracksById.get(connectDraft.trackId);
          if (!sourceTrack || track.status === "REJECTED") {
            showToast("The selected connection is not compatible.", "danger");
            return;
          }
          commit({
            ...
            next.topology = next.topology.filter((relation) => !(relation.sourceAssetId === sourceTrack.id && relation.sourceEndpointIndex === connectDraft.pointIndex && relation.destinationAssetId === track.id));
            next.topology.push({
              id: `TOP-${Date.now()}`,
      ...
      const connectToTurnout = useCallback((turnout) => {
        if (mode !== "CONNECT" || !connectDraft) {
          showToast("Select a source Track endpoint first.", "danger");
          return;
        }
        const sourceTrack = tracksById.get(connectDraft.trackId);
        ...
        commit({
          ...
          source.version = (source.version || 1) + 1;
          source.issues = source.issues.filter((issue) => !["DISCONNECTED", "MISSING_CONNECTION"].includes(issue));
          next.topology = next.topology.filter((relation) => !(relation.sourceAssetId === sourceTrack.id && relation.sourceEndpointIndex === connectDraft.pointIndex && relation.destinationAssetId === turnout.id));
          next.topology.push({
            id: `TOP-${Date.now()}`,
            sourceAssetId: sourceTrack.id,
            sourceEndpointIndex: connectDraft.pointIndex,
      ...
      const finishExtend = useCallback((rawPoint, explicitTarget = null) => {
        if (!extendDraft) {
          showToast("Select the Track endpoint to extend.", "danger");
          return;
        }
        ...
        if (resolved.target?.assetId === track.id) {
          showToast("Extend to a different Track, turnout, or free destination.", "danger");
          return;
        }
        if (samePoint(resolved.point, extendDraft.point)) {
          showToast("Choose a destination away from the selected endpoint.", "danger");
          return;
        }
        const coordinates = track.geometry.coordinates.slice();
        ...
        commit({
          ...
          const touchesExtendedEndpoint = (relation) => relationTouchesEndpoint(relation, "source", track.id, extendDraft.pointIndex, extendedRole) || relationTouchesEndpoint(relation, "destination", track.id, extendDraft.pointIndex, extendedRole);
          const detachedAssetIds = next.topology.filter(touchesExtendedEndpoint).map((relation) => relation.sourceAssetId === track.id ? relation.destinationAssetId : relation.sourceAssetId);
          ...
          target.status = "MODIFIED";
          target.version = (target.version || 1) + 1;
          target.issues = resolved.target?.assetId ? target.issues.filter((issue) => !["DISCONNECTED", "MISSING_CONNECTION"].includes(issue)) : unique([...target.issues, "MISSING_CONNECTION"]);
      ...
      const handleTrackClick = useCallback((event, track) => {
        ...
        if (mode === "MAP") {
          showToast("That geometry is already classified as a Track.", "danger");
          return;
        }
        if (mode === "TRIM") {
          if (selectedIds.length !== 1 || selectedIds[0] !== track.id) {
            showToast("Click the selected Track to trim it.", "danger");
            return;
          }
        ...
        if (mode === "SPLIT") {
          if (selectedIds.length !== 1 || selectedIds[0] !== track.id) {
            showToast("Click the selected Track to split it.", "danger");
            return;
          }
      ...
      const handleUnmappedClick = useCallback((event, geometry) => {
        event.stopPropagation();
        if (mode !== "MAP") {
          showToast("Use Add Track \u2192 Map Existing Geometry to classify this source element.", "info");
          return;
        }
      ...
      const modeInstruction = {
        DRAW: "Draw Track \xB7 click points \xB7 double-click to finish \xB7 Esc cancels",
        MAP: mappingTargetId ? "Complete the Track mapping form in the right panel" : "Map Existing Geometry \xB7 select an unidentified line",
        EDIT_GEOMETRY: "Edit Geometry \xB7 drag points \xB7 double-click to add \xB7 right-click to remove",
        EXTEND: extendDraft ? "Extend \xB7 click a destination" : "Extend \xB7 select a Track endpoint",
        TRIM: "Trim \xB7 click the desired trimming point",
        SPLIT: "Split \xB7 click the split location",
        CONNECT: connectDraft ? "Connect \xB7 choose a destination Track endpoint or turnout" : "Connect \xB7 choose the source Track endpoint",
        PAN: "Pan \xB7 drag the ESP preview"
      }[mode];
      ...
      const sourceLabel = (item) => item.id === "TX-1" ? `KM ${stationBaseChainage(station).toFixed(3)}` : item.id === "TX-2" ? `KM ${(stationBaseChainage(station) + 1.48).toFixed(3)}` : item.text;
      const operationPreview = (() => {
        if (operationConfirm?.type === "JOIN") return /* @__PURE__ */ React.createElement("line", { className: "ptc-operation-line", x1: operationConfirm.pair.a[0], y1: operationConfirm.pair.a[1], x2: operationConfirm.pair.b[0], y2: operationConfirm.pair.b[1] });
        if (operationConfirm?.type === "TRIM") return /* @__PURE__ */ React.createElement("polyline", { className: "ptc-operation-line", "data-tone": "remove", points: operationConfirm.remove.map((point) => point.join(",")).join(" ") });
        if (operationConfirm?.type === "SPLIT") return /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("polyline", { className: "ptc-operation-line", "data-tone": "split-a", points: operationConfirm.left.map((point) => point.join(",")).join(" ") }), /* @__PURE__ */ React.createElement("polyline", { className: "ptc-operation-line", "data-tone": "split-b", points: operationConfirm.right.map((point) => point.join(",")).join(" ") }));
        return null;
      })();
      return /* @__PURE__ */ React.createElement("div", { className: "ptc-shell" }, /* @__PURE__ */ React.createElement("datalist", { id: "ptc-track-names" }, ["UP MAIN LINE", "DN MAIN LINE", "LOOP LINE", "SIDING", "SHUNTING NECK"].map((name) => /* @__PURE__ */ React.createElement("option", { key: name, value: name }))), /* @__PURE__ */ React.createElement("section", { className: "ptc-canvas-col", "aria-label": "Editable ESP Preview" }, /* @__PURE__ */ React.createElement("header", { className: "ptc-preview-head" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-head-copy" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-title" }, /* @__PURE__ */ React.createElement(Icon, { name: "track", size: 16 }), "ESP Preview"), /* @__PURE__ */ React.createElement("div", { className: "ptc-subtitle" }, station?.name || "Station", " \xB7 ", documentState.sourceDocument?.fileName || "Uploaded ESP", " \xB7 Track geometry correction area")), /* @__PURE__ */ React.createElement("div", { className: "ptc-head-actions" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-header-btn", title: "Highlight All Extracted Tracks", "aria-label": "Highlight All Extracted Tracks", "data-active": highlightMode === "ALL" ? "true" : "false", onClick: () => setHighlightMode((current) => current === "ALL" ? "NONE" : "ALL") }, /* @__PURE__ */ React.createElement(Icon, { name: "eye", size: 13 }), /* @__PURE__ */ React.createElement("span", null, "Highlight All Extracted")), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-header-btn", title: "Highlight Tracks with Issues", "aria-label": "Highlight Tracks with Issues", "data-active": highlightMode === "ISSUES" ? "true" : "false", onClick: () => setHighlightMode((current) => current === "ISSUES" ? "NONE" : "ISSUES") }, /* @__PURE__ */ React.createElement(Icon, { name: "alert_tri", size: 13 }), /* @__PURE__ */ React.createElement("span", null, "Highlight Issues")), /* @__PURE__ */ React.createElement("span", { className: "ptc-asset-count" }, activeTrackCount, " assets"))), /* @__PURE__ */ React.createElement("div", { className: "ptc-stage" }, /* @__PURE__ */ React.createElement(
        "svg",
        {
          ref: svgRef,
          className: "ptc-svg",
        ...
        /* @__PURE__ */ React.createElement("defs", null, /* @__PURE__ */ React.createElement("pattern", { id: "ptc-grid-small", width: "36", height: "36", patternUnits: "userSpaceOnUse" }, /* @__PURE__ */ React.createElement("path", { d: "M 36 0 L 0 0 0 36", fill: "none", stroke: "#e8ebef", strokeWidth: "1" }))),
        /* @__PURE__ */ React.createElement("rect", { x: view.x - 500, y: view.y - 500, width: view.w + 1e3, height: view.h + 1e3, fill: "#fbfbf8" }),
        /* @__PURE__ */ React.createElement("rect", { x: view.x - 500, y: view.y - 500, width: view.w + 1e3, height: view.h + 1e3, fill: "url(#ptc-grid-small)" }),
        documentState.sourceDocument?.previewImage && /* @__PURE__ */ React.createElement("image", { className: "ptc-source-image", href: documentState.sourceDocument.previewImage, x: "60", y: "80", width: "1480", height: "370", preserveAspectRatio: "none" }),
        D.STRUCTURES.map((item) => /* @__PURE__ */ React.createElement("g", { key: item.id }, /* @__PURE__ */ React.createElement("rect", { className: "ptc-structure", x: item.x, y: item.y, width: item.w, height: item.h }), /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", x: item.x + item.w / 2, y: item.y + item.h / 2 + 4, textAnchor: "middle" }, item.label))),
        D.PLATFORMS.map((item) => /* @__PURE__ */ React.createElement("g", { key: item.id }, /* @__PURE__ */ React.createElement("rect", { className: "ptc-platform", x: item.x, y: item.y, width: item.w, height: item.h }), /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", x: item.x + item.w / 2, y: item.y + 16, textAnchor: "middle" }, item.label))),
        D.DIMENSIONS.map((item) => /* @__PURE__ */ React.createElement("g", { key: item.id, opacity: ".52" }, /* @__PURE__ */ React.createElement("line", { x1: item.x1, y1: item.y, x2: item.x2, y2: item.y, stroke: "#64748b", strokeWidth: "1" }), /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", x: (item.x1 + item.x2) / 2, y: item.y - 5, textAnchor: "middle" }, item.label))),
        D.TEXT_LABELS.filter((item) => item.text).map((item) => /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", key: item.id, x: item.x, y: item.y, textAnchor: item.anchor }, sourceLabel(item))),
        documentState.unmapped.map((geometry) => /* @__PURE__ */ React.createElement("g", { key: geometry.id }, geometry.paths.map((path, pathIndex) => /* @__PURE__ */ React.createElement("g", { key: `${geometry.id}:${pathIndex}` }, /* @__PURE__ */ React.createElement("polyline", { className: "ptc-unmapped", "data-active": mappingTargetId === geometry.id ? "true" : "false", points: path.map((point) => point.join(",")).join(" ") }), /* @__PURE__ */ React.createElement("polyline", { className: "ptc-unmapped-hit", points: path.map((point) => point.join(",")).join(" "), onClick: (event) => handleUnmappedClick(event, geometry) }))), highlightMode === "ISSUES" && geometry.paths[0]?.[0] && /* @__PURE__ */ React.createElement("g", { transform: `translate(${geometry.paths[0][0][0]},${geometry.paths[0][0][1]})` }, /* @__PURE__ */ React.createElement("circle", { className: "ptc-issue-marker", r: issueRadius }), /* @__PURE__ */ React.createElement("text", { className: "ptc-issue-text", y: ".5" }, "!")))),
        documentState.tracks.map((track) => {
          ...
          return /* @__PURE__ */ React.createElement("g", { key: track.id }, highlighted && /* @__PURE__ */ React.createElement("polyline", { className: "ptc-track-halo", "data-tone": highlightMode === "ISSUES" ? "issue" : "all", points }), /* @__PURE__ */ React.createElement("polyline", { className: "ptc-track", "data-rejected": track.status === "REJECTED" ? "true" : "false", points }), selected && /* @__PURE__ */ React.createElement("polyline", { className: "ptc-selected-line", points }), /* @__PURE__ */ React.createElement(
            "polyline",
            {
              ...
              onDoubleClick: (event) => {
                event.stopPropagation();
                addControlPoint(track.id, eventPoint(event));
              },
              onKeyDown: (event) => {
                if (event.key === "Enter" || event.key === " ") selectTrack(track.id, event.shiftKey);
              }
            }
          ), highlightMode === "ISSUES" && isIssueTrack(track) && issuePoint && /* @__PURE__ */ React.createElement("g", { transform: `translate(${issuePoint[0]},${issuePoint[1]})` }, /* @__PURE__ */ React.createElement("circle", { className: "ptc-issue-marker", r: issueRadius }), /* @__PURE__ */ React.createElement("text", { className: "ptc-issue-text", y: ".5" }, "!")), selected && mode === "EDIT_GEOMETRY" && coordinates.map((point, pointIndex) => /* @__PURE__ */ React.createElement(
            "g",
            {
            ...
            /* @__PURE__ */ React.createElement("circle", { className: "ptc-node-hit", cx: point[0], cy: point[1], r: hitRadius }),
            /* @__PURE__ */ React.createElement("circle", { className: "ptc-control", "data-endpoint": pointIndex === 0 || pointIndex === coordinates.length - 1 ? "true" : "false", cx: point[0], cy: point[1], r: markerRadius, pointerEvents: "none" })
          )), selected && ["EXTEND", "CONNECT"].includes(mode) && [0, coordinates.length - 1].map((pointIndex) => /* @__PURE__ */ React.createElement(
            "g",
            {
              key: `${track.id}:endpoint:${pointIndex}`,
              onClick: (event) => {
                event.stopPropagation();
                chooseEndpoint(track.id, pointIndex);
              }
            },
            /* @__PURE__ */ React.createElement("circle", { className: "ptc-node-hit", cx: coordinates[pointIndex][0], cy: coordinates[pointIndex][1], r: hitRadius }),
            /* @__PURE__ */ React.createElement("circle", { className: "ptc-control", "data-endpoint": "true", cx: coordinates[pointIndex][0], cy: coordinates[pointIndex][1], r: markerRadius, pointerEvents: "none" })
          )), mode === "CONNECT" && connectDraft && !selected && track.status !== "REJECTED" && [0, coordinates.length - 1].map((pointIndex) => /* @__PURE__ */ React.createElement(
            "g",
            {
        ...
        D.TURNOUTS.map((turnout) => /* @__PURE__ */ React.createElement("g", { key: turnout.id, onClick: (event) => {
          if (mode === "CONNECT") {
            event.stopPropagation();
            connectToTurnout(turnout);
          } else if (mode === "EXTEND" && extendDraft) {
            event.stopPropagation();
            finishExtend([turnout.x, turnout.y], { id: turnout.id, assetId: turnout.id, type: "TURNOUT", point: [turnout.x, turnout.y], label: `Turnout ${turnout.id}` });
          }
        } }, /* @__PURE__ */ React.createElement("circle", { className: "ptc-node-hit", cx: turnout.x, cy: turnout.y, r: hitRadius }), /* @__PURE__ */ React.createElement("circle", { className: "ptc-turnout", "data-target": ["CONNECT", "EXTEND"].includes(mode) && (connectDraft || extendDraft) ? "true" : "false", cx: turnout.x, cy: turnout.y, r: markerRadius, pointerEvents: "none" }), /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", x: turnout.x + 9, y: turnout.y - 8 }, turnout.id))),
        D.BUFFER_STOPS.map((item) => /* @__PURE__ */ React.createElement("g", { key: item.id }, /* @__PURE__ */ React.createElement("line", { x1: item.x, y1: item.y - 9, x2: item.x, y2: item.y + 9, stroke: "#263747", strokeWidth: "3" }), /* @__PURE__ */ React.createElement("text", { className: "ptc-source-label", x: item.x + 8, y: item.y + 4 }, item.id))),
        drawPoints.length > 0 && /* @__PURE__ */ React.createElement("polyline", { className: "ptc-operation-line", points: [...drawPoints, cursorPoint ? resolvePoint(cursorPoint).point : drawPoints[drawPoints.length - 1]].map((point) => point.join(",")).join(" ") }),
        drawPoints.map((point, index) => /* @__PURE__ */ React.createElement("circle", { className: "ptc-control", key: `draw:${index}`, cx: point[0], cy: point[1], r: markerRadius })),
        extendDraft && cursorPoint && /* @__PURE__ */ React.createElement("line", { className: "ptc-operation-line", x1: extendDraft.point[0], y1: extendDraft.point[1], x2: resolvePoint(cursorPoint, `${extendDraft.trackId}:${extendDraft.pointIndex}`).point[0], y2: resolvePoint(cursorPoint, `${extendDraft.trackId}:${extendDraft.pointIndex}`).point[1] }),
        connectDraft && cursorPoint && /* @__PURE__ */ React.createElement("line", { className: "ptc-operation-line", x1: connectDraft.point[0], y1: connectDraft.point[1], x2: cursorPoint[0], y2: cursorPoint[1] }),
        operationPreview,
        snapIndicator && /* @__PURE__ */ React.createElement("g", { transform: `translate(${snapIndicator.point[0]},${snapIndicator.point[1]})` }, /* @__PURE__ */ React.createElement("circle", { className: "ptc-snap-marker", r: issueRadius }), /* @__PURE__ */ React.createElement("path", { d: "M-12 0H12M0-12V12", stroke: "var(--accent)", strokeWidth: "1.5" }), /* @__PURE__ */ React.createElement("text", { className: "ptc-snap-label", x: "12", y: "-12" }, "Snap: ", snapIndicator.label)),
        lastConnection && /* @__PURE__ */ React.createElement("g", { transform: `translate(${lastConnection.point[0]},${lastConnection.point[1]})` }, /* @__PURE__ */ React.createElement("circle", { className: "ptc-snap-marker", r: issueRadius }), /* @__PURE__ */ React.createElement("path", { d: "M-5 0l3 3 7-7", fill: "none", stroke: "var(--success-text)", strokeWidth: "2.5" }), /* @__PURE__ */ React.createElement("text", { className: "ptc-snap-label", x: "14", y: "-14" }, lastConnection.label))
      ), modeInstruction && /* @__PURE__ */ React.createElement("div", { className: "ptc-mode-banner" }, /* @__PURE__ */ React.createElement(Icon, { name: mode === "PAN" ? "hand" : mode === "MAP" ? "target" : mode === "CONNECT" ? "branch" : "edit", size: 13 }), modeInstruction), !!selectedIds.length && /* @__PURE__ */ React.createElement("div", { className: "ptc-selection-note" }, /* @__PURE__ */ React.createElement(Icon, { name: "cursor", size: 12 }), selectedIds.length, " Track", selectedIds.length === 1 ? "" : "s", " Selected"), operationConfirm && /* @__PURE__ */ React.createElement("div", { className: "ptc-operation-confirm" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-operation-copy" }, operationConfirm.type === "JOIN" ? "Join selected track segments?" : operationConfirm.type === "SPLIT" ? "Split Track at this location?" : "Trim highlighted Track portion?", /* @__PURE__ */ React.createElement("span", null, operationConfirm.type === "JOIN" ? `${operationConfirm.firstId} \xB7 ${operationConfirm.secondId}` : operationConfirm.trackId)), operationConfirm.type === "TRIM" && /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", onClick: () => setOperationConfirm((current) => ({ ...current, keep: current.remove, remove: current.keep, keepSide: current.keepSide === "start" ? "end" : "start" })) }, "Swap retained side"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", onClick: () => setOperationConfirm(null) }, "Cancel"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-primary": "true", onClick: confirmPendingOperation }, operationConfirm.type === "JOIN" ? "Join" : operationConfirm.type === "SPLIT" ? "Split" : "Trim")), pointMenu && /* @__PURE__ */ React.createElement("div", { className: "ptc-point-menu", style: { left: pointMenu.left, top: pointMenu.top }, role: "menu" }, /* @__PURE__ */ React.createElement("button", { type: "button", role: "menuitem", onClick: () => {
        removeControlPoint(pointMenu.trackId, pointMenu.pointIndex);
        setPointMenu(null);
      } }, /* @__PURE__ */ React.createElement(Icon, { name: "trash", size: 13 }), "Remove Point")), /* @__PURE__ */ React.createElement(
        TrackToolbar,
        {
          mode,
      ...
      )), /* @__PURE__ */ React.createElement("div", { className: "ptc-statusbar" }, /* @__PURE__ */ React.createElement("span", null, "Mode ", /* @__PURE__ */ React.createElement("strong", null, mode.replaceAll("_", " "))), /* @__PURE__ */ React.createElement("span", null, "Snap ", /* @__PURE__ */ React.createElement("strong", null, snapEnabled ? "ON" : "OFF")), /* @__PURE__ */ React.createElement("span", null, "Zoom ", /* @__PURE__ */ React.createElement("strong", null, Math.round(FIT_VIEW.w / view.w * 100), "%")), /* @__PURE__ */ React.createElement("span", null, "Audit ", /* @__PURE__ */ React.createElement("strong", null, documentState.audit.length)), /* @__PURE__ */ React.createElement("span", { className: "ptc-status-spacer" }), /* @__PURE__ */ React.createElement("span", { className: "ptc-save-state", "data-saving": saveState === "Saving..." ? "true" : "false", "data-error": saveState === "Save failed" ? "true" : "false" }, /* @__PURE__ */ React.createElement(Icon, { name: saveState === "Saving..." ? "refresh" : saveState === "Save failed" ? "alert" : "check_circle", size: 12 }), saveState, lastSavedAt && saveState === "Saved" ? ` ${lastSavedAt}` : ""))), /* @__PURE__ */ React.createElement("aside", { className: "ptc-panel", "aria-label": "Track Review Panel" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-panel-head" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-panel-title-row" }, /* @__PURE__ */ React.createElement("div", { className: "ptc-panel-title" }, "Tracks"), /* @__PURE__ */ React.createElement("div", { className: "ptc-panel-actions" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-header-btn", title: "Highlight All Extracted Tracks", "aria-label": "Highlight All Extracted Tracks", "data-active": highlightMode === "ALL" ? "true" : "false", onClick: () => setHighlightMode((current) => current === "ALL" ? "NONE" : "ALL") }, /* @__PURE__ */ React.createElement(Icon, { name: "eye", size: 12 }), "Highlight All"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-header-btn", title: "Highlight Tracks with Issues", "aria-label": "Highlight Tracks with Issues", "data-active": highlightMode === "ISSUES" ? "true" : "false", onClick: () => setHighlightMode((current) => current === "ISSUES" ? "NONE" : "ISSUES") }, /* @__PURE__ */ React.createElement(Icon, { name: "alert_tri", size: 12 }), "Issues", issueCount ? ` ${issueCount}` : ""), /* @__PURE__ */ React.createElement("div", { className: "ptc-add-wrap" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-header-btn", "data-active": addMenuOrigin === "header" ? "true" : "false", onClick: () => setAddMenuOrigin((current) => current === "header" ? null : "header") }, /* @__PURE__ */ React.createElement(Icon, { name: "plus", size: 12 }), "Add"), addMenuOrigin === "header" && /* @__PURE__ */ React.createElement(AddTrackMenu, { onDraw: startDraw, onMap: startMap })))), /* @__PURE__ */ React.createElement("div", { className: "ptc-filter-row" }, /* @__PURE__ */ React.createElement("select", { className: "ptc-filter", "aria-label": "Filter Tracks", value: filter, onChange: (event) => {
        setFilter(event.target.value);
        setSelectedIds([]);
        setExpandedId(null);
      } }, FILTERS.map(([value, label]) => /* @__PURE__ */ React.createElement("option", { key: value, value }, label))), /* @__PURE__ */ React.createElement("span", { className: "ptc-filter-summary" }, filteredTracks.length, " shown \xB7 ", activeTrackCount, " active"))), /* @__PURE__ */ React.createElement("div", { className: "ptc-panel-scroll", ref: panelScrollRef }, mappingTargetId && documentState.unmapped.some((item) => item.id === mappingTargetId) && /* @__PURE__ */ React.createElement(
        MappingForm,
        {
          key: mappingTargetId,
      ...
      ), !filteredTracks.length && /* @__PURE__ */ React.createElement("div", { className: "ptc-empty" }, /* @__PURE__ */ React.createElement(Icon, { name: "inbox", size: 20 }), /* @__PURE__ */ React.createElement("strong", null, "No Tracks in this filter"), /* @__PURE__ */ React.createElement("span", null, "Choose another status or add a missing Track.")), filteredTracks.map((track) => {
        ...
        return /* @__PURE__ */ React.createElement(
          TrackCard,
          {
            ...
            cardRef: (element) => {
              if (element) cardRefs.current.set(track.id, element);
      ...
      }))), /* @__PURE__ */ React.createElement("footer", { className: "ptc-nav" }, /* @__PURE__ */ React.createElement("span", { className: "ptc-nav-context" }, "Track navigation stays in the review panel. These controls move between asset categories."), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", disabled: !canPrevious, onClick: onPrevious }, /* @__PURE__ */ React.createElement(Icon, { name: "chevron_left", size: 13 }), "Previous"), /* @__PURE__ */ React.createElement("button", { type: "button", className: "ptc-inline-btn", "data-primary": "true", disabled: !canNext, onClick: onNext }, "Next", /* @__PURE__ */ React.createElement(Icon, { name: "chevron_right", size: 13 }))), !!rejectIds.length && /* @__PURE__ */ React.createElement(RejectDialog, { tracks: rejectIds.map((id) => tracksById.get(id)).filter(Boolean), onClose: () => setRejectIds([]), onConfirm: confirmReject }), toast && /* @__PURE__ */ React.createElement("div", { className: "ptc-toast", "data-tone": toast.tone, role: "status" }, /* @__PURE__ */ React.createElement(Icon, { name: toast.tone === "danger" ? "alert" : "check_circle", size: 14 }), toast.message));
    };
