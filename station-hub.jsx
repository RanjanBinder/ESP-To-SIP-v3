(() => {
  ...
  const assetConfigFor = (asset, instanceNumber = 1) => {
    ...
    const centralLineLocationFields = [
      ["Station",      instanceNumber === 1 ? "C OF POTHULAPADU(PUPU) STATION BUILDING" : "Central Line Location " + instanceNumber],
      ["Location (KM)", instanceNumber === 1 ? "227.310|km" : `${(227.310 + (instanceNumber - 1) * 0.095).toFixed(3)}|km`],
      ["Chainage (m)",  instanceNumber === 1 ? "227310|m" : `${227310 + (instanceNumber - 1) * 95}|m`],
...
.sh-track-switcher-right { display:flex; align-items:center; gap:4px; flex-wrap:nowrap; }
.sh-track-instance-wrap { display:flex; flex-direction:column; align-items:center; gap:2px; }
.sh-track-delete-btn { width:18px; height:14px; display:grid; place-items:center; border:var(--hairline); border-radius:var(--r-sm); background:transparent; color:var(--ink-400); cursor:pointer; padding:0; transition:background 120ms,color 120ms,border-color 120ms; flex-shrink:0; }
.sh-track-delete-btn:hover { border-color:var(--danger); background:var(--danger-soft); color:var(--danger-text); }
.sh-asset-delete-btn { width:32px; height:32px; display:grid; place-items:center; border:var(--hairline); border-radius:var(--r-md); background:transparent; color:var(--ink-500); cursor:pointer; padding:0; transition:background 120ms,color 120ms,border-color 120ms; flex-shrink:0; }
.sh-asset-delete-btn:hover { border-color:var(--danger); background:var(--danger-soft); color:var(--danger-text); }
.sh-track-nav-btn { width:26px; height:28px; display:grid; place-items:center; border:var(--hairline); border-radius:var(--r-sm); background:transparent; color:var(--ink-500); cursor:pointer; padding:0; transition:background 120ms,color 120ms; flex-shrink:0; }
.sh-track-nav-btn:hover { background:var(--ink-50); }
...
.sh-asset-zoom-btn { width:32px; height:32px; display:grid; place-items:center; border:var(--hairline); border-radius:var(--r-md); background:var(--paper); color:var(--ink-600); cursor:pointer; padding:0; flex-shrink:0; }
.sh-asset-zoom-btn:hover { border-color:var(--ink-300); background:var(--ink-50); color:var(--ink-900); }
...
.sh-measure-input { display:flex; align-items:center; border:var(--hairline); border-radius:var(--r-md); background:var(--ink-50); overflow:hidden; }
.sh-measure-input:focus-within { border-color:var(--accent); background:var(--paper); box-shadow:var(--shadow-focus); }
.sh-measure-input input,.sh-measure-input select,.sh-measure-input input:focus,.sh-measure-input select:focus { border:none; box-shadow:none; background:transparent; border-radius:0; }
...
.sh-field-section { grid-column:1 / -1; display:flex; align-items:center; gap:8px; margin-top:2px; }
.sh-field-section-label { font-size:10px; font-weight:700; letter-spacing:0.06em; color:var(--ink-400); text-transform:uppercase; white-space:nowrap; }
.sh-field-section-line { flex:1; height:1px; background:var(--ink-100); }
.sh-track-chip-field { border:var(--hairline); border-radius:var(--r-md); background:var(--ink-50); padding:6px 8px; display:flex; flex-wrap:wrap; gap:5px; align-items:center; min-height:36px; }
.sh-track-chip-field:focus-within { border-color:var(--accent); background:var(--paper); box-shadow:var(--shadow-focus); }
.sh-track-chip { display:inline-flex; align-items:center; gap:4px; background:var(--ink-900); color:var(--paper); font-size:11px; font-weight:600; border-radius:var(--r-sm); padding:3px 6px 3px 8px; white-space:nowrap; }
.sh-track-chip-remove { display:flex; align-items:center; justify-content:center; width:14px; height:14px; border-radius:2px; border:none; background:rgba(255,255,255,0.15); color:var(--paper); cursor:pointer; padding:0; }
.sh-track-chip-remove:hover { background:rgba(255,255,255,0.3); }
.sh-track-chip-add { display:inline-flex; align-items:center; gap:3px; font-size:11px; font-weight:600; color:var(--accent); background:none; border:none; cursor:pointer; padding:2px 6px; border-radius:var(--r-sm); }
.sh-track-chip-add:hover { background:var(--ink-100); }
.sh-track-drop-wrap { position:relative; }
.sh-drop-backdrop { position:fixed; inset:0; z-index:99; }
.sh-track-drop-menu { position:absolute; top:calc(100% + 4px); left:0; background:var(--paper); border:var(--hairline); border-radius:var(--r-md); box-shadow:0 4px 16px rgba(0,0,0,0.1); min-width:130px; z-index:100; overflow:hidden; }
.sh-track-drop-item { display:block; width:100%; text-align:left; padding:7px 12px; font-size:12px; color:var(--ink-700); background:none; border:none; cursor:pointer; }
.sh-track-drop-item:hover { background:var(--ink-50); }
.sh-track-drop-empty { padding:10px 12px; font-size:12px; color:var(--ink-400); }
...
.sh-step-pill[data-issues="true"] .sh-step-circle { background:var(--warning-soft); color:var(--warning-text); border-color:oklch(0.83 0.09 75); }
...
.sh-step-pill[data-issues="true"] .sh-step-label-name,.sh-step-pill[data-issues="true"] .sh-step-label-count { color:var(--warning-text); }
.sh-step-pill[data-active="true"][data-issues="true"] .sh-step-circle { background:var(--accent); color:var(--paper); border-color:var(--accent); }
.sh-step-pill[data-active="true"][data-issues="true"] .sh-step-label-name { color:var(--accent); }
...
.sh-asset-add-btn { display:inline-flex; align-items:center; gap:5px; height:30px; padding:0 12px; border:var(--hairline); border-radius:var(--r-md); background:transparent; color:var(--ink-600); font-family:var(--font-sans); font-size:12px; font-weight:600; cursor:pointer; transition:background 120ms,color 120ms; flex-shrink:0; }
.sh-asset-add-btn:hover { background:var(--ink-50); color:var(--ink-900); border-color:var(--ink-300); }
.sh-cll-accordion { flex:1; overflow-y:auto; }
.sh-cll-accordion-item { border-bottom:var(--hairline); }
.sh-cll-accordion-item:last-child { border-bottom:none; }
.sh-cll-accordion-header { width:100%; display:flex; align-items:center; justify-content:space-between; gap:10px; padding:12px 16px; background:none; border:none; cursor:pointer; text-align:left; font-family:var(--font-sans); font-size:13px; font-weight:600; color:var(--ink-900); transition:background 120ms; }
.sh-cll-accordion-header:hover { background:var(--ink-50); }
...
.sh-de-canvas-document { position:relative; min-width:100%; width:max-content; }
.sh-de-canvas-document .sh-de-canvas-img { width:100%; min-width:0; }
.sh-de-corrected-tracks { position:absolute; inset:0; width:100%; height:100%; pointer-events:none; overflow:visible; }
.sh-de-corrected-track { fill:none; stroke:#2563eb; stroke-width:2.2; vector-effect:non-scaling-stroke; stroke-linecap:round; stroke-linejoin:round; filter:drop-shadow(0 0 2px rgba(255,255,255,.9)); }
...
`;
  ...
  const AssetInstanceSwitcher = ({ asset, active, onChange, onAdd }) => {
    const PAGE_SIZE = 5;
    const [pageStart, setPageStart] = useStateHub(0);
    const total = asset.count;
    const clampedStart = Math.max(0, Math.min(pageStart, Math.max(0, total - PAGE_SIZE)));
    const pageEnd = Math.min(clampedStart + PAGE_SIZE, total);
  ...
  const ReviewExtractedAssetsPage = ({ station, upload, mode = "review", version, onBack, onFinish, onSaveAndClose, onSipDone }) => {
    ...
    const [toast, setToast] = useStateHub("");
    const [trackReviewMeta, setTrackReviewMeta] = useStateHub({ issueCount: 0, verifiedCount: 0, rejectedCount: 0 });
    const [trackCorrectionDocument, setTrackCorrectionDocument] = useStateHub(null);
    const reviewTrackOptions = React.useMemo(() => {
      const corrected = (trackCorrectionDocument?.tracks || [])
        .filter((track) => track.status !== "REJECTED")
        .map((track) => ({ id: track.id, name: track.name || track.id }));
      return corrected.length ? corrected : TRACK_OPTIONS;
    }, [trackCorrectionDocument]);
    ...
    const useTrackCorrectionWorkspace = activeGroup.type === "Tracks" && !!window.PIMTrackCorrectionWorkspace;
    ...
    const allReviewed = groups.filter((g) => !g.isDigitize).every((g) => statuses[g.type] === "reviewed");
    React.useEffect(() => {
      setStatuses((prev) => prev[activeGroup.type] === "pending" ? { ...prev, [activeGroup.type]: "in_review" } : prev);
    }, [activeGroup.type]);
    const goPrev = () => {
      if (activeGroup.type === "Tracks" && groupIndex > 0) {
    ...
    const goNext = () => {
      if (activeGroup.type === "Tracks" && groupIndex < groups.length - 1) {
        setActiveType(groups[groupIndex + 1].type);
        setActiveInstance(1);
      } else if (activeInstance < activeGroup.count) {
        setActiveInstance(activeInstance + 1);
    ...
    const handleTrackStatusChange = React.useCallback((nextStatus, countOverride) => {
      const payload = typeof nextStatus === "string" ? { status: nextStatus } : nextStatus || {};
      setTrackReviewMeta((current) => ({ ...current, ...payload }));
      const nextCount = Number(countOverride ?? payload.count ?? payload.total ?? payload.assetCount);
      if (Number.isFinite(nextCount) && nextCount >= 0) {
        setAssetCounts((current) => ({ ...current, Tracks: nextCount }));
      }
      if (!payload.status) return;
      const normalizedStatus = String(payload.status).trim().toLowerCase().replace(/[\s-]+/g, "_");
      const reviewStatus = {
        completed: "reviewed",
        complete: "reviewed",
        verified: "reviewed",
        reviewed: "reviewed",
        has_issues: "correction",
        hasissues: "correction",
        issues: "correction",
      ...
      if (reviewStatus) {
        setStatuses((current) => ({ ...current, Tracks: reviewStatus }));
      }
    }, []);
    const markReviewed = () => {
      setStatuses((prev) => ({ ...prev, [activeGroup.type]: "reviewed" }));
    ...
    const measuredUnits = ["m", "km", "cm"];
    const splitMeasuredValue = (value, fallbackUnit) => {
      const source = String(value || "");
      const unitMatch = source.match(/\b(km|cm|m)$/i);
      const amount = source.includes("|") ? source.split("|")[0] : source.replace(/\s*(km|cm|m)$/i, "");
      const unit = source.includes("|") ? source.split("|")[1] : unitMatch?.[1];
      return { amount: amount.trim(), unit: (unit || fallbackUnit).trim().toLowerCase() };
    };
    ...
    const getTrackOption = (trackId) => reviewTrackOptions.find((option) => option.id === trackId);
    ...
    const removeAssetAt = (instanceNumber) => {
    ...
    const updateFieldForInstance = (n, label, value) => {
      const key = activeGroup.type + "-" + n;
      const d = getDraftForInstance(n);
      setDrafts((prev) => ({ ...prev, [key]: { ...d, [label]: value } }));
    };
    const getCllAccordionHeader = (n) => {
      const d = getDraftForInstance(n);
      const station = d["Station"] || "Central Line " + n;
      const ch = splitMeasuredValue(d["Chainage (m)"] || "", "m");
      const km = splitMeasuredValue(d["Location (KM)"] || "", "km");
      const fcsb = splitMeasuredValue(d["FCSB (m)"] || "", "m");
      const parts = [station];
      if (ch.amount) parts.push("Ch: " + ch.amount + "m");
      if (km.amount) parts.push("(Km " + km.amount + ")");
      if (fcsb.amount) parts.push("(FCSB:" + fcsb.amount + "m)");
      return parts.join(", ");
    };
    ...
    const renderBridgeAccordion = () => /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("div", { className: "sh-bridge-accordion" }, Array.from({ length: activeGroup.count }, (_, idx) => {
      ...
      const availableTrackOptions = reviewTrackOptions.filter((option) => !bridgeTrackIds.includes(option.id));
    ...
    const openEspEditor = () => {
      if (!allReviewed) {
        setToast("Resolve every asset review issue before generating the Digital ESP.");
    ...
    const saveGeneratedDigital = () => {
      if (onFinish) onFinish({
        close: true,
        trackCorrectionDocument,
        correctedTracks: trackCorrectionDocument?.tracks?.filter((track) => track.status !== "REJECTED") || []
      });
    };
    ...
    if (isDigitizeStep && digitalGenerated) {
      ...
      return /* @__PURE__ */ React.createElement("div", { className: "sh-digital-editor-full" },
        ...
        /* @__PURE__ */ React.createElement("div", { className: "sh-de-canvas", ref: deCanvasRef },
          /* @__PURE__ */ React.createElement("div", { className: "sh-de-canvas-document" },
            ...
            trackCorrectionDocument && /* @__PURE__ */ React.createElement("svg", { className: "sh-de-corrected-tracks", viewBox: "0 60 1600 700", preserveAspectRatio: "none", "aria-label": "Corrected Track geometry" },
              trackCorrectionDocument.tracks.filter((track) => track.status !== "REJECTED").map((track) => /* @__PURE__ */ React.createElement("polyline", { key: track.id, className: "sh-de-corrected-track", points: track.geometry.coordinates.map((point) => point.join(",")).join(" ") }))
            )
          )
        ),
  ...
  const StationHubPage = ({ station: initialStation, onBack, onEditorModeChange }) => {
    ...
    const handleGenerateDigitalEsp = (options = {}) => {
      ...
      const nextVersion = {
        ...
        trackCorrectionDocument: options.trackCorrectionDocument || null,
        trackAssetCount: options.correctedTracks?.length || 0
      };
